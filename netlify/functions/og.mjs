...
function wrapperColor(hex) {
  const [r, g, b] = hexToRgb(hex);
  const lightness = (r + g + b) / 765;
  const whiteMix = Math.min(0.55, Math.max(0.18, 0.42 - (lightness - 0.5) * 0.35));
  return mixHex(hex, "#FFFFFF", whiteMix);
}

function inkColor(hex) {
  return mixHex(hex, "#000000", 0.58);
}
...
function renderCrayon(color, name, x, y, scale) {
  const wrapper = wrapperColor(color);
  ...
  const labelText = escapeXml(truncateName(name));

  return `
    <g transform="translate(${x} ${y}) scale(${scale})">
      <path fill="${wrapper}" d="${wrapperPath}"/>
      <path fill="${ink}" fill-rule="evenodd" clip-rule="evenodd" d="${CRAYON_DETAIL}"/>
      <path fill="${ink}" d="${labelPath}"/>
      <path fill="${color}" d="${tipPath}"/>
      <text x="900" y="74" text-anchor="end" font-family="-apple-system, system-ui, sans-serif" font-size="46" font-weight="600" fill="#111111">${labelText}</text>
    </g>
  `;
}

export function buildOgSvg(context) {
  const colors = context.colors.slice(0, MAX_COLORS_PER_PALETTE);
  const crayonCount = Math.max(colors.length, 1);
  const gap = 12;
  const crayonBaseHeight = 121;
  const crayonBaseWidth = 1172;
  const stackAreaTop = 48;
  const stackAreaBottom = 582;
  const stackAreaHeight = stackAreaBottom - stackAreaTop;
  const stackAreaLeft = 430;
  const stackAreaWidth = 720;

  let scale = 0.44;
  const totalHeight = crayonCount * crayonBaseHeight * scale + (crayonCount - 1) * gap;
  if (totalHeight > stackAreaHeight) {
    scale = (stackAreaHeight - (crayonCount - 1) * gap) / (crayonCount * crayonBaseHeight);
  }

  const crayonHeight = crayonBaseHeight * scale;
  const crayonWidth = crayonBaseWidth * scale;
  const stackHeight = crayonCount * crayonHeight + (crayonCount - 1) * gap;
  const startY = stackAreaTop + (stackAreaHeight - stackHeight) / 2;
  const startX = stackAreaLeft + (stackAreaWidth - crayonWidth) / 2;

  const crayons = colors
    .map((color, index) =>
      renderCrayon(
        color,
        colorNameForHex(color),
        startX,
        startY + index * (crayonHeight + gap),
        scale,
      ),
    )
    .join("");
  ...
  const accentColor = colors[0] ?? "#1A4876";

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630" role="img" aria-label="Crayons app preview">
  <rect width="1200" height="630" fill="#FFFFFF"/>
  <rect x="430" y="0" width="770" height="630" fill="#F7F7F7"/>
  <rect x="430" y="0" width="1" height="630" fill="#E8E8E8"/>
  <rect x="72" y="168" width="56" height="8" rx="4" fill="${accentColor}"/>
  <text x="72" y="248" font-family="-apple-system, system-ui, sans-serif" font-size="88" font-weight="700" fill="#1A1A1A">Crayons</text>
  <text x="72" y="328" font-family="-apple-system, system-ui, sans-serif" font-size="30" fill="#555555">${subtitle}</text>
  <text x="72" y="378" font-family="-apple-system, system-ui, sans-serif" font-size="22" fill="#888888">Curated Crayola palettes</text>
  ${crayons}
</svg>`;
}
